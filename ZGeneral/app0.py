...
def dict_to_markdown(question, Sub, data, iter_count,main_count,out_dir=None):
    # Initialize an empty list to store DataFrames
    # Append row
    if iter_count == 1:
        temp = f"{question}\n {Sub}"
    ...
    # ---- Auto-save to log directory ----
    if out_dir is not None:
        csv_path = f"logs/{out_dir}/table_snapshot.csv"
        xlsx_path = f"logs/{out_dir}/table_snapshot.xlsx"
        html_path = f"logs/{out_dir}/table_snapshot.html"
    ...
    with col2:
        ...
        if Button:
            ...
            if pdf is None:
                ...
                for i, q in enumerate(st.session_state.questions_data):
                    ...
                    for j in range(len(sub_q)):
                        ...
                        if resp.startswith("```json") and resp.endswith("```"):
                            ...
                            df_to_write=dict_to_markdown(f'{q['question']}', f'{sub_q[j]}', final_response, iteration_counter,Main_Counter,out_dir=out_dir )
                    ...
                    if len(weights) >= Main_Counter:
                        ...
                        if len(w) == len(r):
                            #160ct
                            sum_weight = sum([wi for wi in w])
                            score = sum([wi * ri for wi, ri in zip(w, r)])
                            score = score/sum_weight
                        else:
                    ...
                    main_weight_sum += main_weights[Main_Counter-1]
                ...
                if "table_rows" in st.session_state and len(st.session_state["table_rows"]) > 0:
                    ...
                    st.download_button(
                        ...
                        data=csv,
                        file_name=f"SRS_Verification_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                        mime="text/csv"
                    )
                    ...
                    st.download_button(
                        ...
                        file_name=f"SRS_Verification_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx",
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                    )
                else:
                    st.info("Table not yet generated.")
